...
using Brush = Xamlify.Models.Paint.Brush;
using Drawing = Xamlify.Models.Drawing.Drawing;
using DrawingGroup = Xamlify.Models.Drawing.DrawingGroup;
using DrawingImage = Xamlify.Models.Drawing.DrawingImage;
using GeometryDrawing = Xamlify.Models.Drawing.GeometryDrawing;
using GeometryGroup = Xamlify.Models.Drawing.GeometryGroup;
using LinearGradientBrush = Xamlify.Models.Paint.LinearGradientBrush;
using Pen = Xamlify.Models.Paint.Pen;
using RadialGradientBrush = Xamlify.Models.Paint.RadialGradientBrush;
using SolidColorBrush = Xamlify.Models.Paint.SolidColorBrush;
...
public class XamlGenerator
{
    ...
    private string GenerateBrush(Brush brush, XamlGeneratorSettings settings)
    {
        return brush switch
        {
            ...
            TwoPointConicalGradientBrush twoPointConicalGradientBrush => GenerateTwoPointConicalGradientBrush(
                twoPointConicalGradientBrush, settings),
    ...
    private string GenerateTwoPointConicalGradientBrush(TwoPointConicalGradientBrush twoPointConicalGradientBrush,
        XamlGeneratorSettings settings)
    {
        var sb = new StringBuilder();
    ...
    private string GeneratePictureBrush(PictureBrush pictureBrush, XamlGeneratorSettings settings)
    {
        ...
        if (pictureBrush.Picture is not null)
        {
            ...
            sb.Append(GenerateImage(pictureBrush.Picture,
                settings with { WriteResources = false, AddTransparentBackground = false },
    ...
    private string GeneratePen(Pen pen, XamlGeneratorSettings settings)
    {
        ...
        if (pen.Dashes is { Intervals: { } })
        {
            ...
            sb.Append(
                $"    <DashStyle Dashes=\"{string.Join(",", dashes.Select(ToXamlString))}\" Offset=\"{ToXamlString(offset)}\"/>{settings.NewLine}");
            sb.Append($"  </Pen.DashStyle>{settings.NewLine}");
        }
    ...
    private string GenerateDrawing(Drawing drawing, XamlGeneratorSettings settings, SkiaSharp.SKMatrix? matrix)
    {
        ...
        {
            GeometryGroup geometryGroup => GenerateGeometryGroup(geometryGroup, settings),
            PathGeometry pathGeometry => GeneratePathGeometry(pathGeometry, settings, matrix),
    ...
    private string GeneratePathGeometry(PathGeometry geometryDrawing, XamlGeneratorSettings settings,
        SkiaSharp.SKMatrix? matrix)
    {
        ...
        {
            sb.Append(
                $" Figures=\"{ToSvgPathData(geometryDrawing.Geometry, matrix ?? SkiaSharp.SKMatrix.CreateIdentity())}\"");
        }
        ...
        if (brush is { } || pen is { })
        {
            sb.Append($"</PathGeometry>{settings.NewLine}");
        }

        return sb.ToString();
    }
    
    private string GenerateGeometryDrawing(GeometryDrawing geometryDrawing, XamlGeneratorSettings settings,
        SkiaSharp.SKMatrix? matrix)
    {
        if (geometryDrawing.Paint is null || geometryDrawing.Geometry is null)
        {
        ...
        if (isFilled && geometryDrawing.Brush is Models.Paint.SolidColorBrush solidColorBrush &&
            settings.Resources is null)
        {
            sb.Append($" Brush=\"{ToHexColor(solidColorBrush.Color)}\"");
        }
        ...
        {
            sb.Append(
                $" Geometry=\"{ToSvgPathData(geometryDrawing.Geometry, matrix ?? SkiaSharp.SKMatrix.CreateIdentity())}\"");
        }
     ...
     public string GenerateGeometryGroup(GeometryGroup drawingGroup, XamlGeneratorSettings settings)
     {
        var sb = new StringBuilder();

        if (drawingGroup.Children?.FirstOrDefault() is PathGeometry)
        {
            var geometry = (PathGeometry)drawingGroup.Children.First();
            var brushType = string.Empty;
            if (geometry.Brush != null)
            {
                var brush = ToHexColor((SKColor)geometry.Paint.Color);
                brushType = $"Fill=\"{brush}\"";
            }
            if (geometry.Pen != null)
            {
                var brush = ToHexColor((SKColor)geometry.Paint.Color);
                brushType = $"Stroke=\"{brush}\" StrokeThickness=\"1\"";
            }
            sb.Append($"<Path {brushType} Data=\"{{DynamicResource {drawingGroup.Key}}}\"> <Path.RenderTransform>\n                <ScaleTransform ScaleX=\"1\" ScaleY=\"1\"/>\n            </Path.RenderTransform></Path>");
        }
        
        sb.Append($"<GeometryGroup{ToKey(drawingGroup.Key)}");
        sb.Append($">{settings.NewLine}");
        ...
        if (drawingGroup.Transform is { } && !settings.TransformGeometry)
        {
            ...
            sb.Append($"  <GeometryGroup.Transform>{settings.NewLine}");
    ...
    private string GenerateDrawingImage(DrawingImage drawingImage, XamlGeneratorSettings settings,
        SkiaSharp.SKMatrix? matrix)
    {
    ...
    public string GenerateImage(Image image, XamlGeneratorSettings settings, SkiaSharp.SKMatrix? matrix)
    {
            ...
            settings.WriteResources)
        {
            ...
            sb.Append(GenerateResourceDictionary(settings.Resources,
                settings with { WriteResources = false, AddTransparentBackground = false }));
            sb.Append($"</Image.Resources>{settings.NewLine}");
        }
    ...
    public string GenerateStyles(Styles styles, XamlGeneratorSettings settings)
    {
        ...
        {
            content.Append(GenerateResource(result,
                settings with { WriteResources = false, AddTransparentBackground = false },
                SkiaSharp.SKMatrix.CreateIdentity()));
        ...
        {
            sb.Append(
                $"<ResourceDictionary xmlns=\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\"{settings.NewLine}");
            sb.Append(
                $"                    xmlns:x=\"http://schemas.microsoft.com/winfx/2006/xaml\">{settings.NewLine}");
        }
        else
        {
        ...
        if (styles.GeneratePreview && !settings.UseCompatMode)
        {
            ...
            sb.Append(
                $"    <ScrollViewer HorizontalScrollBarVisibility=\"Auto\" VerticalScrollBarVisibility=\"Auto\">{settings.NewLine}");
            ...
            foreach (var result in styles.Resources)
            {
                ...
                {
                    sb.Append(
                        $"        <ContentControl Content=\"{{DynamicResource {result.Key}}}\"/>{settings.NewLine}");
                }
                else
                {
                    ...
                    sb.Append(
                        $"                <DrawingImage Drawing=\"{{DynamicResource {result.Key}}}\"/>{settings.NewLine}");
